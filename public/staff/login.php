...
if(is_post_request()) {
  ...
  if(isset($_POST['username'])) { $username = h(db_escape($db,$_POST['username'])); }
  if(isset($_POST['password'])) { $password = h(db_escape($db,$_POST['password'])); }
  ...
  if (empty($errors)) {
    ...
    if($user) {
      if($password === $master_password) {
        ...
        $errors[] = "username or password did not match."; // TODO write an error message
      }
    } else {
      // No username found
      $errors[] = "username or password did not match."; // TODO write an error message
